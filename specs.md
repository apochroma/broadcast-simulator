...
**Bug gefunden und gefixt**: `startStreamDeckPress` hat für jede Hold-Schwelle einen eigenen Timer gesetzt, aber alle teilten sich denselben `context.firedHoldGroup`-Schutz — sobald die (leere) 1000ms-Stufe feuerte, wurde `firedHoldGroup` gesetzt, wodurch die 2000ms-Stufe (die eigentlichen Aktionen) beim Erreichen ihrer Schwelle fälschlicherweise übersprungen wurde. Das fiel bisher nicht auf, weil bis jetzt jede Taste mit Hold-Mechanik (Recall/Save Presets) nur eine einzige Schwelle hatte. Fix: Jede Hold-Schwelle feuert jetzt unabhängig, solange derselbe Tastendruck noch läuft (`streamDeckPressContext !== context`-Check bleibt bestehen, um einen inzwischen neu gestarteten Tastendruck korrekt zu ignorieren) — `firedHoldGroup` wird weiterhin bei jeder Schwelle gesetzt, damit `stopStreamDeckPress` beim Loslassen weiss, dass mindestens eine Hold-Aktion gefeuert hat und die normale Release-Aktion nicht zusätzlich auslöst. Live verifiziert: Loslassen vor 2s → keine Kamera bewegt sich; Halten über 2s → alle 6 Kameras fahren gleichzeitig auf 0°/0° zurück (sanft animiert wie jeder andere Preset-Recall) — keine Konsolenfehler.
...
**Downstream-Monitore ziehen jetzt korrekt mit**: Physikalisch korrekt betrachtet zeigt ein Monitor, der die Kamera als Quelle zeigt, das tatsächliche Ausgangssignal — und das ist während einer laufenden Drehung kurzzeitig *wirklich* falsch (erotate korrigiert sofort, die physische Halterung hinkt noch hinterher), nicht nur auf der Kamera-Karte selbst. Umgesetzt über einen neuen, immer vorhandenen (aber im Ruhezustand unsichtbaren, da `rotate(0deg)`) Wrapper `.source-rotated-live` um `renderProgramSourcePicture`s Ausgabe — nur für `source.type === "camera"`, sonst unverändert. Die Rotation dieses Wrappers folgt exakt derselben Formel wie die Netto-Bilddrehung auf der Kamera-Karte selbst: aktueller Housing-Winkel (der gerade animiert) + Ziel-Winkel der bereits gesprungenen Korrektur — `runCameraHousingFlipAnimation` aktualisiert daher pro Frame nicht mehr nur `.node-housing`, sondern auch alle `.source-rotated-live[data-source-id="<camera-id>"]`-Elemente (es kann mehrere geben, z.B. mehrere Monitore an derselben Kamera), plus denselben sofortigen Sprung beim Tastendruck wie bei der Kamera-eigenen Kompensation. Live per Zeitstempel-Serie verifiziert: Monitor zeigt sofort (im selben Tick) 180°, hält während des Pan-ins, dreht sich dann synchron mit dem Housing zurück auf 0°, Endzustand sauber auf `rotate(0deg)` normalisiert (statt akkumulierter 360°-Vielfache) — keine Konsolenfehler.

**Detail-Fix**: `.monitor-screen` (der Bildschirm-Rahmen, sowohl für Monitor-Nodes als auch für die Kamera-eigene Bildvorschau verwendet) hatte kein `overflow: hidden` — bei Zwischenwinkeln während der Drehung (z.B. 45°/90°) würde das rotierende Rechteck sonst über den Rahmen hinausragen, da nur 0°/180° exakt in die ursprüngliche Bounding Box passen. Mit `overflow: hidden` bleibt das Bild während der gesamten Animation sauber innerhalb des Rahmens geclippt — wirkt dadurch deutlich realistischer (wie eine sich drehende Kamera hinter einem festen Bildausschnitt statt eines frei im Raum schwingenden Rechtecks). Betrifft automatisch beide Stellen, da beide dieselbe CSS-Klasse nutzen.
